...
(function ($) {
  ...
  $("#api-connection-settings-form").on("submit", async function (e) {
    ...
    try {
      const response = await postData(wcrpd_ajax_object.ajax_url, {
        action: "save_wcrpd_options",
      ...
      if (response.success) {
        ...
        if (response.data.field_errors) {
          Object.keys(response.data.field_errors).forEach((fieldName) => {
            ...
            $(`.server-error-feedback[data-for="${fieldName}"]`)
              .text(errorMessage)
              .show();
          });
        }
        displayErrorMessage(
          $(".error-message"),
          "An error occurred: " + response.data
        );
      }
    } catch (error) {
      displayErrorMessage(
  ...
  // Reset settings
  $("#wcrpd-reset-settings-button").on("click", async function (e) {
    e.preventDefault();

    try {
      const response = await postData(wcrpd_ajax_object.ajax_url, {
        action: "wcrpd_delete_options",
        nonce: $('input[name="nonce"]').val(),
      });
